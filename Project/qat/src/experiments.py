...
class Experiment:
    ...
    def run(self):
        ...
        for epoch in range(self.wandb.config["general"]["epochs"]):
            self.training_step(log = log)
            self.validation_step(log = log)
        ...
        self.validation_step(run_on_test_dataset_instead=True, log=True)
    ...
    def training_step(self, log=False):
        # Training step
        self.model.train()
        summed_loss = 0.0
        correct = 0
    ...
    def validation_step(self, run_on_test_dataset_instead=False, log=False):
        # Validation step
        self.model.eval()
        summed_loss = 0.0
        correct = 0
